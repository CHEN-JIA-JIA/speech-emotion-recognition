...
def train_emotion_model(
    ...
    # 用于记录每个epoch的训练指标，后面要写入到JSON
    epoch_logs = []
    if epoch_log_path and os.path.exists(epoch_log_path):
        # 如果文件已存在，可考虑先读取再append，也可直接覆盖
        pass

    for epoch in range(epochs):
        current_time_str = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        # 只在 epoch 开始时显示时间，后面指标对齐
        print(f"\n=== Start Epoch [{epoch+1:3d}/{epochs}] at {current_time_str} ===")
        ...
        # 将本轮训练的指标放进 epoch_logs，后续存 JSON
        epoch_logs.append({
            "epoch": epoch + 1,
        ...
        # 每个epoch结束也可即时写入文件（避免训练中断数据丢失）
        if epoch_log_path:
            with open(epoch_log_path, 'w', encoding='utf-8') as f:
                json.dump(epoch_logs, f, indent=4)

    return emotion_model, optimizer, scheduler, (
        final_epoch_loss, final_acc, final_prec, final_rec, final_f1, best_f1, best_epoch
    )
...
if __name__ == "__main__":
    ...
    resumed_emotion_model, resumed_optimizer, resumed_scheduler, final_metrics = train_emotion_model(
        ...
        # 新增：指定日志文件路径
        epoch_log_path = r"E:\Speech emotion recognition\work_dir\training_epoch_log.json"
    )
